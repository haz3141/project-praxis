...
- **Fast Re-entry:** After any interruption or setback, the system helps users quickly re-engage (“pick up where you left off”). The system *recovers* users, it never punishes.
...
- **Motivation Fragility:** Sustaining effort over time is hard (especially with ADHD or executive-function issues). Strict streaks or punitive rewards often backfire, causing burnout or disengagement. Users need encouraging feedback loops without shame or addiction. Praxis must boost intrinsic motivation gently, e.g., celebrating *consistency, not volume*.

These problems are grounded in behavioral science. For example, one study found that creating a task list and priorities boosts work engagement – but only if interruptions are managed. Another highlights that ADHD-related executive function deficits “adversely” affect a person’s ability to begin, sustain, and complete tasks.  Praxis is designed for these challenges: it channels planning energy into a simple loop and adds supportive overlays only if they truly help.
...
**Each stage must guard against drop-off:** e.g., if too many tasks accumulate in Inbox, user abandon; if review is skipped, learning stops. The system thus gently reminds or auto-defers tasks to avoid paralysis. Research shows that explicit planning of tasks raises engagement and performance; Praxis institutionalizes that into a ritual.

### Modules (Optional Layers)
...
- **Anti-Gaming Guardrails:** We avoid common pitfalls (negative study results):
...
The guiding principle: *“quiet dopamine”*. Rewards are subtle, ambient, and always supplement the core feeling of accomplishment, not replace it.

## 5. Scope Boundaries

### MVP / V1 / V2 Scope Boundaries
...
- **Weekly Completion Rate (North Star):**
  ...
  \]
  This measures how well plans turn into done work. A high rate (e.g. > 70%) indicates the loop is working. Research suggests even small increases here correlate with engagement.

- **Activation Rate:** (onboarding success)
  \(\frac{\text{Users who create 3+ plans in first week}}{\text{New sign-ups}}\).
  (Goal: high percentage, e.g. >50% by Day 7.)  Some industry benchmarks: Day-1 retention ~33% for productivity apps, Day-30 ~10%. We’ll target exceeding those via quick time-to-value.

- **DAU/MAU Ratio:**
  \(\text{DAU/MAU} = \frac{\text{7-day active users}}{\text{30-day active users}}\).
  A healthy product sees 25–30% (as a rough benchmark from consumer apps).  >25% suggests habit formation.
...
- **Re-entry Success Rate:** (resilience metric)
  ...
  Measures how often a user picks up planning again (goal: as high as possible). Users with ADHD often struggle to restart, so we aim for >50% coming back within a week of break.
...
- **Interruption & Recovery:** If a user had a long break (illness, vacation), on login we surface a “Recovery Plan” prompt (e.g. “Welcome back! Would you like to revive last week’s plan?”). This is validated by research that short mental breaks improve efficiency; we bake that in by design.
...
### Anti-Features (Explicit Prohibitions)
