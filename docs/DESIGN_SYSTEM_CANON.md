...
Two approved theme modes share one semantic token model:

1. `calm` (default)
- Primary mode for Planner routes.
- Low-noise, high legibility, restrained motion.
- Matches execution-first doctrine.

2. `neon_holo` (opt-in, secondary)
- Neon holographic glassmorphism variant.
...
### 4.2 Neon Holographic Mode (Opt-In)
...
Required guardrails for `neon_holo`:
...
- [ ] Map existing tokens in `packages/design-tokens/src/tokens` to semantic names above.
- [ ] Add `mode=calm` as default token output.
- [ ] Add `mode=neon_holo` token output behind explicit opt-in.
- [ ] Run contrast checks for both modes on Planner core components.
